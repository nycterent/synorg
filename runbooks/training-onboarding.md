...
![Preemption timeline of a training job: node A checkpoints every 5 minutes until a preemption (SIGTERM); a 120 s grace flushes a final checkpoint; after Kueue re-admits, the job resumes on a fresh node B from the last checkpoint, losing at most one interval (≤5 min)](../assets/diagrams/preemption-timeline.svg){ .diagram }
