...
## Deploy path
PR → `make validate` (helm template → kubeconform → kyverno → rendered diff) → capability-tier lane (auto / human review / deny) → merge → ArgoCD sync. No `kubectl` writes outside break-glass. Humans and agents are distinct principals; see `docs/agent-interface.md`.
## Quick start
```bash
...
Conventions (pool names, taints, priority classes, required labels): `docs/conventions.md`.
## Roadmap
...
- Teardown hardening: sweep EKS-created security groups and stray Karpenter instances inside `phase_down` (three manual interventions on 2026-07-19 prove the need), and rename the `lending_reclaim_window_active` misnomer (see `docs/glossary.md`)
