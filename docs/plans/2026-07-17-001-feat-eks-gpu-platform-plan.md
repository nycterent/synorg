...
2. **"90% idle" means allocation-idle**, so whole-node lending is the right fix. *Revisit:* U9 telemetry shows kernel-idle-under-allocation during peak → separate utilization workstream (batching, right-sizing, async I/O pipelines), not more lending — lending cannot recover kernel-idle, only the workload can. External evidence for this failure mode: Synthesia's VAE-decoder inference on G7e ran at 82% kernel utilization while fully allocated (frame-save I/O stalls), recovered to 99.9% by an async CUDA copy-stream pipeline (`docs/solutions/2026-07-17-kernel-idle-vs-allocation-idle.md`).
