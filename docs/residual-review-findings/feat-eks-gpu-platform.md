...
- ~~**#19 P2 `tests/e2e/assertions.sh:208` — assert_reclaim can pass via close-time untaint, not actual reclaim** (driven closesAt +11m clears taints before the +15m ramp deadline regardless of drain). Require reclaim evidence (events / deleted NodeClaims) or move closesAt past the deadline.~~ RESOLVED (2026-07-18): both halves applied — the driven schedule now closes AFTER the ramp deadline (waves +5/+8/+11, deadline +15, closesAt +20 via `E2E_CLOSE_MINUTES`; waves < ramp < close guarded in `drive_schedule_now`), so inside the measured window only reclaimWaves can clear taints; and the assertion requires positive controller-log wave evidence — `action=reclaim_wave … reclaiming=` firing lines plus `action=nodeclaim_deleted … wave=` NodeClaim deletions (the close path logs `reason=window_close`, a due-but-empty wave has no `reclaiming=` token, so neither can satisfy the greps). Log reads are bounded (`--since` from the drive epoch, `--tail=5000`); unreachable controller logs are a loud FAIL, never a skip. (adversarial)
- ~~**#20 P2 `tests/e2e/assertions.sh:224` — scrub "new instance" proof satisfied by any second pool node.** Snapshot all pool providerIDs at lend time; require one outside the entry set.~~ RESOLVED (2026-07-18): assert_lend snapshots every non-null lendable-pool providerID at lend time (`$E2E_STATE_DIR/pool-entry-providerids.txt`); assert_scrub passes only when a Ready pool node's providerID is OUTSIDE that entry set (a genuinely new EC2 instance — a pre-existing sibling can never match), FAILs naming the entry-set size when no new instance appears within the bounded wait, and keeps the explicit old!=new equal-id guard (node-scrub.md abort case). (adversarial)
